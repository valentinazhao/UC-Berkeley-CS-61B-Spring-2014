...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    int prev = ints[0], index = 0;
    for(int i = 1; i < ints.length; i++) {
      if(i == (ints.length - 1) && index != i) {
        //System.out.println("666666");
        Arrays.fill(ints, index + 1, ints.length, -1);
      }
      if(ints[i] == prev) {
        continue;
      }
      ...
      ints[index] = ints[i];
      prev = ints[i];
      System.out.println("prev is" + prev + " *** " + "i =" + i + " *** " + "index = " + index);
      //System.out.println("666666");
    }

    
    for(Integer tmp: ints) {
      System.out.println("ints is" + tmp);
    }
  }
